...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="tei:body/tei:pb">
        <a>
            <xsl:attribute name="href">images/<xsl:value-of 
                select="@facs"/></xsl:attribute><img><xsl:attribute 
                    name="src">images/thumbs/<xsl:value-of 
                        select="@facs"/></xsl:attribute></img>
        </a>
        <xsl:apply-templates/>
    </xsl:template>
